...
ttagit.port.on('confirmLogin',function(url){
	var oReq = new XMLHttpRequest();


	oReq.onload = function(e) {
	  var arraybuffer = oReq.response; // not responseText
	  console.log(url,"CONFIRM LOGIN THING HERE **********************",arraybuffer);
	  ttagit.port.emit('loginConfirmed',oReq.response);
	}
	oReq.open("GET", url, true);
	oReq.send();
...
ttagit.port.on('saveData',function(data){	
	ss.storage.ttagitData = data;
});

ttagit.port.on('getAccessToken',function(dataType){
	self.port.emit('receiveToken',ss.storage.ttagitData['ACCESS_TOKEN_STORAGE_KEY']);
});

ttagit.port.on('getAccessTokenSecret',function(dataType){
	self.port.emit('receiveTokenSecret',ss.storage.ttagitData['ACCESS_TOKEN_STORAGE_KEY']);
});

ttagit.port.on('getUserID',function(dataType){
	self.port.emit('receiveUserID',ss.storage.ttagitData['ACCESS_TOKEN_STORAGE_KEY']);
});
